...
const NextPageButton = (props: buttonProps) => {
    return(
        <div className={`${props.margin} hidden md:block relative bottom-0 opacity-0 hover:opacity-100 w-full h-2 animation-fade duration-700`}>
            <button className="absolute bottom-0 inset-x-[42%] mb-12 p-2 bg-metal rounded-full w-[6%] border-2 border-opacity-75 border-[#1C2B2D] cursor-pointer select-none">
              <Link href={props.nextPageReference} className="w-full flex flex-row items-center justify-center ">
