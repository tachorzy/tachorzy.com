    ...
    return( 
        <div className={ManropeMedium.className + " absolute right-0 w-[40%] lg:xl:ml-24 bg-metal py-12 px-16 rounded-md mr-[17rem] h-full"}>
            <form className="flex flex-col gap-y-1">
                    ...
                    return (
                        <label className="flex flex-col my-2 duration-500 focus-within:p-1 focus-within:pl-3 focus-within:border-l-4 border-sandstone border-inherit rounded">
                            <span className="text-dune text-xl">{label}</span>
                                {label == "Your message:" 
                                    ? <textarea name="message" placeholder={placeholderText} className=" pl-1 py-0.5 group bg-shark h-auto duration-700 transition-[height] w-full focus-within:h-28 border-b-[3px] text-xl placeholder-mudbrick ring-0 outline-0 text-sandstone focus-within:border-slate border-sandstone border-inherit"></textarea> 
                ...
                <button className=" rounded bg-sandstone hover:bg-dusty text-slate font-semibold py-2 w-1/4 mt-6">
                    {"Submit!"}
                </button>
            </form>
        </div>
